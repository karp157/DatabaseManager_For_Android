package com.lifetracker.database;//add your package name here example: package com.example.dbm;
...
public class AndroidDatabaseManager extends Activity implements OnItemClickListener
{
	...
	//int below line Change the text 'yourCustomSqlHelper' with your custom sqlitehelper class name.Donot change the variable
	// name dbm
	MySQLiteHelper dbm;
	...
	protected void onCreate(Bundle savedInstanceState)
	{
		...
		//the main linear layout to which all tables spinners etc will be added.In this activity every element is created
		// dynamically  to avoid using xml file
		...
		// a query which returns a cursor with the list of tables in the database.We use this cursor to populate spinner in the
		// first row
					...
					if (c2 != null) {
						...
						spinnertable.setOnItemSelectedListener((new OnItemSelectedListener()
						{
							...
							public void onItemSelected(AdapterView<?> parentView, View selectedItemView, int position, long id)
							{
											...
											if (!isFinishing()) {
												...
												new AlertDialog.Builder(AndroidDatabaseManager.this).setTitle("Are you sure ?")
														.setMessage("Pressing yes will remove " + indexInfo.table_name + " " +
																"table" +
																" from database").setPositiveButton("yes",
														new DialogInterface.OnClickListener()
														{
															// when user confirms by clicking on yes we drop the table by
															...
															public void onClick(DialogInterface dialog, int which)
															{
																...
																if (tempc.getString(0).equalsIgnoreCase("Success")) {
																	...
																	//if there is any error we displayd the error message at the
								...
								//when user selects to drop the table the below code in if block will be executed
								if (spinnertable.getSelectedItem().toString().equals("Delete this table")) {    // an alert
									// dialog to confirm user selection
									runOnUiThread(new Runnable()
									{
										@Override
										public void run()
											...
											if (!isFinishing()) {
												...
												new AlertDialog.Builder(AndroidDatabaseManager.this).setTitle("Are you sure?")
														.setMessage("Clicking on yes will delete all the contents of " +
																indexInfo.table_name + " table from database").setPositiveButton("yes",
														new DialogInterface.OnClickListener()
														{

															// when user confirms by clicking on yes we drop the table by
															// executing delete table query
															public void onClick(DialogInterface dialog, int which)
															{
																...
																if (tempc.getString(0).equalsIgnoreCase("Success")) {
																	tvmessage.setBackgroundColor(Color.parseColor("#2ecc71"));
																	tvmessage.setText(indexInfo.table_name + " table content " +
								...
								if (spinnertable.getSelectedItem().toString().equals("Add row to this table")) {
									...
									RelativeLayout.LayoutParams addnewparams = new RelativeLayout.LayoutParams(RelativeLayout
									...
									for (int i = 0; i < addnewrownames.size(); i++) {
										...
										RelativeLayout.LayoutParams rll = new RelativeLayout.LayoutParams(RelativeLayout
										...
										public void run()
										{
											if (!isFinishing()) {
												new AlertDialog.Builder(AndroidDatabaseManager.this).setTitle("values")
														...
														new DialogInterface.OnClickListener()
														{
															// after entering values if user clicks on add we take the values
															// and run a insert query
															public void onClick(DialogInterface dialog, int which)
															{
	...
	public void updateDeletePopup(int row)
	{
		...
		runOnUiThread(new Runnable()
		{
			...
			{
				if (!isFinishing()) {
					new AlertDialog.Builder(AndroidDatabaseManager.this).setTitle("values").setView(updaterowsv).setCancelable
							(false).setPositiveButton("Ok", new DialogInterface.OnClickListener()
					{

						//this code will be executed when user changes values of edit text or spinner and clicks on ok
						// button
						public void onClick(DialogInterface dialog, int which)
						{
							...
							if (spinner_value.equalsIgnoreCase("Update this row")) {
								...
								for (int i = 0; i < columnames.size(); i++) {
									TextView tvc = columnames.get(i);
									if (!value_string.get(i).equalsIgnoreCase("null") && !(value_string.get(i).trim().length()
											== 0)) {
